...
# Read environment variables for CosmosDB
COSMOSDB_URI = os.environ.get("COSMOSDB_URI")
COSMOSDB_MASTER_KEY = os.environ.get("COSMOSDB_MASTER_KEY")
COSMOSDB_DATABASE = os.environ.get("COSMOSDB_DATABASE", "databaseName")
COSMOSDB_CONTAINER = os.environ.get("COSMOSDB_CONTAINER", "ChatGPTPluginContainer")
...
class CosmosDBDataStore(DataStore):
    def __init__(self, client: redis.Redis, redisearch_schema, cosmos_container_client: ContainerProxy):
        self.client = client
        self._schema = redisearch_schema
        self._cosmos_container_client = cosmos_container_client
    ...
    async def init(cls, **kwargs):
        ...
        # Create the database and container, handling the exception if a 
        # container with the name already exists in the database.
        cosmos_client = CosmosClient(COSMOSDB_URI, credential=COSMOSDB_MASTER_KEY)
        try:
            database = await cosmos_client.create_database(id=COSMOSDB_DATABASE)
        except cosmos_exceptions.CosmosResourceExistsError:
            database = cosmos_client.get_database_client(database=COSMOSDB_DATABASE)

        try:
            container = await database.create_container(
                id=COSMOSDB_CONTAINER, partition_key=PartitionKey(path="/id")
            )
        except cosmos_exceptions.CosmosResourceExistsError:
            container = database.get_container_client(COSMOSDB_CONTAINER)

        return cls(client, redisearch_schema, container)
    ...
    def _get_cosmos_chunk(self, chunk: DocumentChunk) -> dict:
        """
        Convert DocumentChunk into a JSON object for storage
        in CosmosDB.
        ...
        Returns:
            dict: JSON object for storage in CosmosDB.
        """
    ...
    async def _cosmos_delete(self, id: str):
        """
        ...
        """
        try:
            await self._cosmos_container_client.delete_item(id, id)
    ...
    async def _upsert(self, chunks: Dict[str, List[DocumentChunk]]) -> List[str]:
        ...
        for doc_id, chunk_list in chunks.items():
            ...
            # And write the content to the CosmosDB container
            first_metadata.pop("document_id")
            cosmosdoc = {
                "id": doc_id,
                "metadata": first_metadata,
                "chunks": [self._get_cosmos_chunk(chunk) for chunk in chunk_list]
            }
            await self._cosmos_container_client.upsert_item(cosmosdoc)
    ...
    async def _query(
        ...
        for query in queries:
            ...
            for doc in query_response.docs:
                ...
                chunk_id = doc_json["chunk_id"]
                cosmos_doc = await self._cosmos_container_client.read_item(id, id)
                chunks = cosmos_doc["chunks"]
                text = next(chunk["text"] for chunk in chunks if chunk["chunk_id"] == chunk_id)
    ...
    async def delete(
        ...
        if delete_all:
            try:
                ...
                logging.info(f"Deleted all documents from redis successfully")

                async for item in self._cosmos_container_client.query_items(
                    query='SELECT * FROM {} c'.format(COSMOSDB_CONTAINER),
                ):
                    await self._cosmos_delete(item.id)
                logging.info(f"Deleted all documents from CosmosDB successfully")
        ...
        if filter:
            ...
            if filter.document_id:
                try:
                    ...
                    await self._cosmos_delete(filter.document_id)
        ...
        if ids:
            try:
                ...
                for document_id in ids:
                    ...
                    logging.info(f"Deleting {document_id} from CosmosDB")
                    await self._cosmos_delete(filter.document_id)
